    ...
    private String getEditForm(DtoBO dtoBO) {
        ...
        for (DtoAttrBO dtoAttrBO : dtoBO.getAttrList()) {
            ...
            if (!StringUtils.isEmpty(selectStr)) {
                ...
                csb.appendln("</el-form-item>");
            } else if ("Date".equalsIgnoreCase(dtoAttrBO.getTypeStr())) {
                csb.appendln("<el-form-item label=\"【】\">", remName);
                csb.appendln("<el-date-picker");
                csb.appendln("v-model=\"row.【】\"",dtoAttrBO.getNameStr());
                csb.appendln("type=\"date\"");
//                csb.appendln("value-format=\"yyyy-MM-dd HH:mm:ss\"");
                csb.appendln("placeholder=\"选择日期\">");
                csb.appendln("</el-date-picker>");
                csb.appendln("</el-form-item>");
            } else if ("Boolean".equalsIgnoreCase(dtoAttrBO.getTypeStr())) {
                csb.appendln("<el-form-item label=\"【】\">", remName);
                csb.appendln("<el-radio-group v-model=\"row.【】\">", dtoAttrBO.getNameStr());
